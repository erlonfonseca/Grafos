/******************************************************************************
                             heap.c
   Este programa implementa um heap MINIMO 
   Com uma implementação statica, podendo ser mudado para uma implementação dinamica 
******************************************************************************/
#include <malloc.h>
#include <stdio.h>
#define true 1
...
void heapSort(HEAP * h){
  int tamanho = h->tamanhoAtual;
  int i, temp;
  construirHeapMin(h);  // se o arranjo jah for um heap, nao precisa desta linha
  for (i=tamanho;i>1;i--){
     temp = h->A[1];
     h->A[1] = h->A[i];
     h->A[i] = temp;
